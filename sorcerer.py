    ...
    def magic_missile(self):
        ...
        if confirmation_board("lunch missile wich deals 1-5 dmg"):
            ...
            if lvl_s:
                    ...
                    listdeg=[[self.action.dice(5),1,0,0,24,1,0,0] for n in range(lvl_s)] 
    ...
    def firebolt(self):
        ...
        if self.actionP>0:
            if confirmation_board("lunch firebolt.s deals between 1 and 10 damage"):
                listdeg.append([self.action.dice(10),1,0,0,24,1,0,0])
                self.actionP-=1 
                if self.level>=5:
                    listdeg.append([self.action.dice(10),1,0,0,24,1,0,0])
        if listdeg:
    ...
    def fireball(self):
        ...
        if confirmation_board("cost lvl 2 spell deals between 6-36 damage in area"):
            ...
            if self.spells_slots[1]>0 and self.actionP>0:
                ...
                listdeg.append([deg,1,1,1,24,1,10+3+self.score("con"),1]) 
                self.spells_slots[1]-=1
                self.actionP-=1
