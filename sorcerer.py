...
class Sorcerer(Perso_game):
    ...
    def magic_missile(self):
        ...
        if lvl_s:
                ...
                listdeg=[[self.action.dice(4)+1,1,0,0,2,1,0,0] for n in range(lvl_s)] 
    ...
    def firebolt(self):
        ...
        dc (si 0 pas de saving throw possible),type de saving thow (si 0 à dc 0 au type)]"""
        if self.actionP>0:
            listdeg.append([self.action.dice(9)+1,1,0,0,2,1,0,0])
            self.actionP-=1 
            if self.level>=5:
                listdeg.append([self.action.dice(10),1,0,0,2,1,0,0])
        else:
            running=board_error("no spell slot anymore or no action left")
        if listdeg:
            return listdeg
    ...
    def fireball(self):
        ...
        if self.spells_slots[1]>0 and self.actionP>0:
            ...
            listdeg.append([deg,1,0,1,2,1,10+3+self.score("con"),1]) 
            self.spells_slots[1]-=1
            self.actionP-=1
    ...
    def caracter_sheet(self):
        ...
        perso=pygame.transform.scale(wizard_hide['idle_1.png'],(board.get_width()//3,board.get_height()//2))
    ...
    def select_bonus(self, choice):
        if choice==0:
            return self.convertSP()
        elif choice==1:
            return self.convertSpellS()
        elif choice==2:
            return self.quick_spell()