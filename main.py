...
class Menu():
    ...
    def affichage_set_point(self,x,y,cap,display):
        ...
        if self.bouton_click(button_2,display):         
            if cap <= 7 or cap >= 19  :
                ...
                return cap -1
        return cap
    def afficherinventaire(self,perso,pos_x,pos_y,display,Is_perso = True,Is_shop = False):
            ...
            if Is_perso:
                ...
                self.draw_text('Pieces : 10',ColderWeather_small,WHITE,display,pos_x//2-text_width//2,pos_y//2+LARGEUR//6)
                self.draw_text('Poids : 10 / 10',ColderWeather_small,WHITE,display,pos_x//2-LARGEUR//3,pos_y//2+LARGEUR//6)
    ...
    def shop_print(self,perso1,perso2,just_print = False):
        ...
        while running:
            display.fill(LIGHT_GREY)
            items = None
            items = self.afficherinventaire(perso1,display.get_width(),display.get_height()//2,display,False,Is_shop=True)
            if items != None:
            ...
            self.afficherinventaire(perso2,display.get_width(),1.5*display.get_height(),display)
