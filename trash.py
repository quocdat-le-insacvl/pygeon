...
class Case():
    ...
    def print_contains(self):
        if self.in_case != None:
            ...
            screen.blit(self.in_case.display, (self.cordo()[0]-self.in_case.img.get_width(
            )//2, self.cordo()[1]-self.in_case.display.get_height()+self.in_case.img.get_height()//2))
...
class Game():
    ...
    def main_game(self):
        ...
        player_rect = pygame.Rect(player_x, player_y, walk_bottom['walk_bottom_' + str(
            1) + '.png'].get_width(), walk_bottom['walk_bottom_' + str(1) + '.png'].get_height())
        display_joueurs = pygame.Surface((walk_bottom['walk_bottom_' + str(
            1) + '.png'].get_width(), walk_bottom['walk_bottom_' + str(1) + '.png'].get_height()))
        ...
        # Create a link from game to Map
        self.map = Map
        display_with_nature = pygame.Surface((18000, 10000))
        display_with_nature.set_colorkey(BLACK)
        display_with_nature, collision, collision_change_camera, tree_position, collision_entity = print_map(
            Map, display_with_nature)
        ...
        display_with_nature = print_nature(
            Map, display_with_nature, tree_position)
        ...
        list_mooving_entity[1] = self.move_entity(list_mooving_entity[1], [
                                                  0, 0], collision_entity, collision, pieds_mask, player_rect)
        ...
        while running:
            ...
            if swap:
                ...
                screen.blit(display_joueurs, (center_x +
                                              player_rect.x, center_y+player_rect.y))
            ...
            if f < 150:
                list_mooving_entity[0] = self.move_entity(list_mooving_entity[0], [
                                                          2, -1], collision_entity, collision, pieds_mask, player_rect)
                list_mooving_entity[2] = self.move_entity(list_mooving_entity[2], [
                                                          2, -1], collision_entity, collision, pieds_mask, player_rect)
                #list_mooving_entity[1] = self.move_entity(list_mooving_entity[1],[2,-1],collision_entity,collision,pieds_mask,player_rect)
            elif f > 150:
                list_mooving_entity[0] = self.move_entity(
                    list_mooving_entity[0], [-2, 1], collision_entity, collision, pieds_mask, player_rect)
                list_mooving_entity[2] = self.move_entity(
                    list_mooving_entity[2], [-2, 1], collision_entity, collision, pieds_mask, player_rect)
                #list_mooving_entity[1] = self.move_entity(list_mooving_entity[1],[-2,1],collision_entity,collision,pieds_mask,player_rect)
            if f == 300:
                f = 0

            print_mooving_entity(
                screen, list_mooving_entity, center_x, center_y)
            # self.ajouter_case_connue(player_rect,case_connue)
            
            # Brouillard !!!!
            # self.print_fog(player_rect,screen,case_connue,center_x,center_y)

            screen.blit(pieds, (center_x+player_rect.x+20,
                                center_y+player_rect.y+perso_img.get_height()-15))
            ...
            if entity_near:
                draw_text("Press I for interact %s" % entity.name,
                ...
                if interact:
                    entity = self.find_nearest_entity(
                        player_rect, list_mooving_entity)
            ...
            draw_text(" player_x : %i,player_y : %i, %i , %i, FPS : %i" % (len(collision_entity),
                                                                 list_mooving_entity[0].interaction[0][1], collision_entity[0][0], collision_entity[0][1], clock.get_fps()), ColderWeather, WHITE, screen, 100, 100)
            '''Animation Frame'''
            ...
            """ Déplacement joueurs"""
            if mouvement[0]:
                player_rect, swap, entity_near = self.move_player(player_rect, [
                                                                  10, -5], collision, collision_change_camera, pieds_mask, perso_img, center_x, center_y, swap, collision_entity, entity_near)
                display_joueurs.fill(LIGHT_GREY)
                display_joueurs.blit(
                    walk_top['walk_top_' + str(int(n)) + '.png'], (0, 0))
            elif mouvement[1]:
                player_rect, swap, entity_near = self.move_player(
                    player_rect, [-10, +5], collision, collision_change_camera, pieds_mask, perso_img, center_x, center_y, swap, collision_entity, entity_near)
                display_joueurs.fill(LIGHT_GREY)
                display_joueurs.blit(
                    walk_bottom['walk_bottom_' + str(int(n)) + '.png'], (0, 0))
            elif mouvement[2]:
                player_rect, swap, entity_near = self.move_player(
                    player_rect, [+10, +5], collision, collision_change_camera, pieds_mask, perso_img, center_x, center_y, swap, collision_entity, entity_near)
                display_joueurs.fill(LIGHT_GREY)
                display_joueurs.blit(
                    walk_right['walk_right_' + str(int(n)) + '.png'], (0, 0))
            elif mouvement[3]:
                player_rect, swap, entity_near = self.move_player(
                    player_rect, [-10, -5], collision, collision_change_camera, pieds_mask, perso_img, center_x, center_y, swap, collision_entity, entity_near)
                display_joueurs.fill(LIGHT_GREY)
                display_joueurs.blit(
                    walk_left['walk_left_' + str(int(n)) + '.png'], (0, 0))
            else:
                display_joueurs.fill(LIGHT_GREY)
                display_joueurs.blit(
    ...
    def print_fog(self, rect, display, case_connue, center_x, center_y):
        # display.fill(LIGHT_GREY)
        ...
        for j in range(-5, 5, 1):
            for i in range(-5, 5, 1):
                if not case_connue.__contains__([ligne_colonne[0]+i, ligne_colonne[1]+j]):
                    case_connue.append(
                        [ligne_colonne[0]+i, ligne_colonne[1]+j])
        j, i = 0, 0
        for j in range(-13, 13, 1):
            for i in range(-13, 13, 1):
                if not case_connue.__contains__([ligne_colonne[0]+i, ligne_colonne[1]+j]):
                    display.blit(pixel_red, (((ligne_colonne[1]+j-ligne_colonne[0]-i)*190//2 +
                                              9000+center_x, (ligne_colonne[1]+j+ligne_colonne[0]+i)*190//4+center_y)))
    ...
    def move_entity(self, entity, mouvement, collision_entity, collision, pieds_mask, joueurs):
        ...
        if collision.__contains__((entity.pos_x - pixel_red.get_width()//2+entity.img.get_width()//2, entity.pos_y-int(pixel_red.get_height()//1.5)+entity.img.get_height())):
            collision.remove((entity.pos_x - pixel_red.get_width()//2+entity.img.get_width(
            )//2, entity.pos_y-int(pixel_red.get_height()//1.5)+entity.img.get_height()))
        if not pieds_mask.overlap(pixel_mask, ((entity.pos_x - pixel_red.get_width()//2+entity.img.get_width()//2 + mouvement[0])-joueurs.x, entity.pos_y-int(pixel_red.get_height()//1.5)+entity.img.get_height() + mouvement[1]-(joueurs.y+130))):
            for x in entity.interaction:
                ...
                collision_entity.append(
                    (int(x[0])+mouvement[0], int(x[1])+mouvement[1]))
            collision.append((entity.pos_x - pixel_red.get_width()//2+entity.img.get_width()//2 +
                              mouvement[0], entity.pos_y-int(pixel_red.get_height()//1.5)+entity.img.get_height() + mouvement[1]))
            ...
            collision.append((entity.pos_x - pixel_red.get_width()//2+entity.img.get_width(
            )//2, entity.pos_y-int(pixel_red.get_height()//1.5)+entity.img.get_height()))
            return entity
        """def move_entity(self,entity,mouvement,collision_entity,collision,pieds_mask,joueurs):
    ...
    def find_nearest_entity(self, player_rect, list_entity):
        distance = abs(
            player_rect.x - list_entity[0].pos_x) + abs(player_rect.y - list_entity[0].pos_y)
        ...
        for i in range(len(list_entity)):
            if abs(player_rect.x - list_entity[i].pos_x) + abs(player_rect.y - list_entity[i].pos_y) < distance:
                distance = abs(
                    player_rect.x - list_entity[i].pos_x) + abs(player_rect.y - list_entity[i].pos_y)
